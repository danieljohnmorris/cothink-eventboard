...
document.on("dom:loaded", function() {
  function validateForm(form) {
    var required = form.select('*[required]').pluck('name');
    var data = $H(form.serialize(true));

    var errors = required.reject(function(name) { return data.get(name) });

    errors = errors.map(function(name) { return form.down('*[name="' + name + '"]') });
